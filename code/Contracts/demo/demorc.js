...
var encryptedApiKey = [
    'f68d2a32cf17b1312c6db3f236a38c94', 
    '4c9f92f6ec1e2a20a1413d0ac1b867a3']
...
function log (e, c) {
    if (!e && c) { console.log(c.address); }
    else {console.log(e);}
}

/*
function setup_tc() {
  var tc = TownCrier.new({from: minerAddr, data: contracts.TownCrier.code, gas: gasCnt},
          function (e, tc) {
              log(e, tc);
          });
  checkWork();
...
function setup_tc() {
    var tc = TownCrier.new({from: minerAddr, data: contracts.TownCrier.code, gas: gasCnt}, function(e, c) {
        if (!e){
            if (c.address) {console.log("Town Crier created at: " + c.address)} 
        }
        else {console.log(e)}})
...
function createSteamTrade(apiKey, item, price) {
  var tradeContract = SteamTrade.new(
          ...
          function(e, c) {
              if (!e && c.address) {console.log('SteamTrade created at: ' + c.address)} 
              else {console.log(e)}
          });
  checkWork();
...
function purchase(contract, steamId, delay) {
  ...
  // delay = 60, typically
  contract.purchase.sendTransaction(
          steamId, 
          delay, 
          {from: buyerAddr, value: 1e+18 + (55 * 5e+13), gas: gasCnt})
