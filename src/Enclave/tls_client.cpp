...
//HttpResponse https_client(HttpRequest &httpRequest) {
//    int ret = 0;
...
//     */
//
//    LL_LOG("connecting over TCP: %s:%s...", httpRequest.host.c_str(), httpRequest.port.c_str());
//
//    if ((ret = mbedtls_net_connect(&server_fd, httpRequest.host.c_str(), httpRequest.port.c_str(),
//                                   MBEDTLS_NET_PROTO_TCP)) != 0) {
...
//#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
//    if ((ret = mbedtls_ssl_conf_max_frag_len(&conf, MBEDTLS_SSL_MAX_FRAG_LEN_NONE)) != 0) {
//        LL_CRITICAL("mbedtls_ssl_conf_max_frag_len returned %d", ret);
//        goto exit;
//    }
//#endif
//
...
//
//#if defined(MBEDTLS_X509_CRT_PARSE_C)
//    if ((ret = mbedtls_ssl_set_hostname(&ssl, httpRequest.host.c_str())) != 0) {
//        LL_CRITICAL("mbedtls_ssl_set_hostname returned %d", ret);
//        goto exit;
//    }
//#endif
//
...
//    {
//        string http_request;
//        http_request += string("GET ") + httpRequest.url;
//        if (isHttp11 && !http_request.find("HTTP/1.1")) {
//            http_request += " HTTP/1.1";
//        }
//        http_request += string("\r\n");
//
//        for (vector<string>::iterator it = httpRequest.headers.begin(); it != httpRequest.headers.end(); it++)
//        {
//            http_request += (*it) + "\r\n";
//        }
//
//        http_request += "Accept: text/html\r\n";
//        if (isHttp11 && !http_request.find("Host:")) {
//            http_request += "Host: " + string(httpRequest.host);
//        }
//
//        size_t tail_len = (int) strlen(GET_REQUEST_END);
//
//        http_request += string(GET_REQUEST_END);
//
//        for (int written = 0, frags = 0; written < http_request.size(); written += ret, frags++) {
//            while ((ret = mbedtls_ssl_write(&ssl,
//                                            reinterpret_cast<const unsigned char *>(http_request.c_str()) + written,
//                                            http_request.size() - written)) <= 0) {
//                if (ret != MBEDTLS_ERR_SSL_WANT_READ &&
//                    ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
//                    mbedtls_printf("  mbedtls_ssl_write returned -%#x", -ret);
//                    goto exit;
...
//            }
//        }
//        dump_buf("Request: ", (const unsigned char *) http_request.c_str(), http_request.length());
//    }
//
//    /*
...
//            } else {
//                if (ret == 0) {
//                    dump_buf("RESPONSE", buf.buf, buf.length);
//                    break;
//                }
//
//
...
//                dump_buf("Received: ", buf.buf, buf.length);
//
//                n_parsed = http_parser_execute(&parser, &settings, reinterpret_cast<const char *>(buf.buf),
//                                               (size_t) ret);
//                if (parser.upgrade) {
//                    LL_CRITICAL("Not support yet");
//                    ret = ERR_ENCLAVE_SSL_CLIENT;
//                    goto exit;
//                } else if (n_parsed != ret) {
//                    LL_CRITICAL("Error: received %d bytes and parsed %d of them", ret, n_parsed);
...
//
//        if (cb_data.eof == 0) {
//            LL_CRITICAL("receiving buffer (%dB) is not big enough", buf.cap);
//        }
//
//        string response_headers(reinterpret_cast<const char *>(buf.buf),
//                                cb_data.header_length == 0 ? buf.length : cb_data.header_length);
...
//        dump_buf("RESP Headers:", (const unsigned char *) response_headers.c_str(), response_headers.length());
//
//        if (!isHttp11 && parser.status_code == 404 && response_headers.find("HTTP/1.1")) {
//            isHttp11 = true;
//            goto send_request;
//        }
//
//        // copy the data out before releasing the buffer (which will be released upon exiting its scope)
//        httpResponse.status_code = parser.status_code;
//        httpResponse.content.insert(0, (const char*) buf.buf + cb_data.header_length, buf.length);
//    }
//
//
//    /*
...
//    ret = 0;
//
//    LL_LOG("closed %s:%s", httpRequest.host, httpRequest.port);
//
//    /*
//     * Cleanup and exit
//     */
