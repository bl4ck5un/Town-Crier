...
int ABI_Bytes::encode(bytes &out)
{
  ...
  // padding left
  for (size_t i = 0; i < ROUND_TO_32(this->_data.size()) - this->_data.size();
       i++) {
    out.push_back(0);
  }
...
//{
//    if (append_as_uint256(out, this->items.size(), 4) != 0)
//    {printf_sgx("Error! enc(out, int) return non-zero!\n"); return -1;} for
//    (size_t i = 0; i < this->items.size(); i++)
//    {
...
int ABI_Generic_Array::encode(bytes &out)
{
  ...
  for (i = 0; i < this->items.size(); i++) {
    if (this->items[i]->dynamic()) {
      ...
      if (append_as_uint256(out, head_len_sum + tail_len_sum, sizeof(size_t)) !=
          0) {
        return -1;
      }
    } else {
