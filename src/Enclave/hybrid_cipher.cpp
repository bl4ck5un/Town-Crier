...
//
// Permission to use the "TownCrier" software ("TownCrier"), officially docketed
// at the Center for Technology Licensing at Cornell University as D-7364,
// developed through research conducted at Cornell University, and its
// associated copyrights solely for educational, research and non-profit
...
// non-commercial.
//
// Those desiring to incorporate TownCrier software into commercial products or
// use TownCrier and its associated copyrights for commercial purposes must
// contact the Center for Technology Licensing at Cornell University at 395 Pine
// Tree Road, Suite 310, Ithaca, NY 14850; email: ctl-connect@cornell.edu; Tel:
// 607-254-4698; FAX: 607-254-5454 for a commercial license.
//
// IN NO EVENT SHALL CORNELL UNIVERSITY BE LIABLE TO ANY PARTY FOR DIRECT,
// INDIRECT, SPECIAL, INCIDENTAL, OR CONSEQUENTIAL DAMAGES, INCLUDING LOST
// PROFITS, ARISING OUT OF THE USE OF TOWNCRIER AND ITS ASSOCIATED COPYRIGHTS,
// EVEN IF CORNELL UNIVERSITY MAY HAVE BEEN ADVISED OF THE POSSIBILITY OF SUCH
// DAMAGE.
//
// THE WORK PROVIDED HEREIN IS ON AN "AS IS" BASIS, AND CORNELL UNIVERSITY HAS
...
// Foundation (NSF grants CNS-1314857, CNS-1330599, CNS-1453634, CNS-1518765,
// CNS-1514261), a Packard Fellowship, a Sloan Fellowship, Google Faculty
// Research Awards, and a VMWare Research Award.
//
...
void HybridEncryption::storePubkey(const mbedtls_ecp_group *grp,
                                   const mbedtls_ecp_point *p,
                                   ECPointBuffer buf)
{
  size_t olen;
  int ret = mbedtls_ecp_point_write_binary(
      grp, p, MBEDTLS_ECP_PF_UNCOMPRESSED, &olen, buf, PUBLIC_KEY_SIZE);
  if (ret != 0 || olen != PUBLIC_KEY_SIZE) {
    throw runtime_error("mbedtls_ecp_point_write_binary failed");
  }
}

void HybridEncryption::loadPubkey(const mbedtls_ecp_group *grp,
                                  mbedtls_ecp_point *p,
                                  const uint8_t *buf)
{
...
string HybridEncryption::encode(const HybridCiphertext &ciphertext)
{
  ...
  tmp_buf.insert(tmp_buf.end(),
                 ciphertext.user_pubkey,
                 ciphertext.user_pubkey + ciphertext.USER_PUBKEY_LEN);
  tmp_buf.insert(tmp_buf.end(),
                 ciphertext.aes_iv,
                 ciphertext.aes_iv + ciphertext.AES_IV_LEN);
  tmp_buf.insert(tmp_buf.end(),
                 ciphertext.gcm_tag,
                 ciphertext.gcm_tag + ciphertext.GCM_TAG_LEN);
...
HybridCiphertext HybridEncryption::decode(const string &cipher_b64)
{
  ...
  ciphertext.data.insert(
      ciphertext.data.end(), cipher, cipher + o_len - ciphertext.HEADER_LEN);
...
//  ret = mbedtls_ecdh_gen_public(&ecdh_ctx_tc.grp, &ecdh_ctx_tc.d,
//  &ecdh_ctx_tc.Q,
...
void HybridEncryption::queryPubkey(ECPointBuffer pubkey)
{
  ...
  ret = mbedtls_mpi_read_string(
      &g_secret_hybrid_key, 16, PREDEFINED_HYBRID_SECKEY);
  if (ret != 0) {
                                     ...
                                     vector<uint8_t> &cleartext)
{
  if (g_secret_hybrid_key.p == NULL) {
    throw runtime_error(
        "hybrid key not provisioned yet. Run queryPubkey() first");
  }
                                     ...
                                     vector<uint8_t> &cleartext)
{
  ...
  ret = mbedtls_ecdh_compute_shared(&ctx_tc.grp,
                                    &ctx_tc.z,
                                    &ctx_tc.Qp,
                                    &ctx_tc.d,
  ...
  mbedtls_debug_print_mpi(
      &dummy_ssl_ctx, 0, __FILE__, __LINE__, "derived secret", &ctx_tc.z);
...
string HybridEncryption::hybridEncrypt(const ECPointBuffer tc_pubkey,
                                       const uint8_t *data,
                                       size_t data_len)
{
  HybridCiphertext ciphertext;
                                     ...
                                     HybridCiphertext &ciphertext)
{
  ...
  // generate an ephemeral key
  ret = mbedtls_ecdh_gen_public(&ctx_user.grp,
                                &ctx_user.d,
                                &ctx_user.Q,
                                mbedtls_ctr_drbg_random,
                                &ctr_drbg);
  ...
  ret = mbedtls_ecdh_compute_shared(
      &ctx_user.grp, &ctx_user.z, &ctx_user.Qp, &ctx_user.d, NULL, NULL);
  CHECK_RET_GO(ret, cleanup);

  mbedtls_debug_print_mpi(
      &dummy_ssl_ctx, 0, __FILE__, __LINE__, "derived secret", &ctx_user.z);
  ...
  aes_gcm_256_enc(
      aes_key, aes_iv, data, data_len, ciphertext.gcm_tag, ciphertext.data);
...
int HybridEncryption::secretToPubkey(const mbedtls_mpi *seckey,
