...
RequestParser::RequestParser(const string &input) : raw_request(input) {
  LL_LOG("request parser get is %s", input.c_str());
  if (input.size() < RequestParser::REQUEST_MIN_LEN) {
    throw invalid_argument("input string is too short");
  }
  // 0x00 - 0x20 bytes : id
...
uint8_t *RequestParser::getData() const { return data; }
