...
For simplicity, we assume a single instance of \engine, although our architecture could scale up to multiple enclaves and even multiple hosts.
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item It forwards datagrams from the \encname to the blockchain.
\end{enumerate}
...
(We discuss certificate checking in Appendix~\ref{sec:impl} of the online version~\cite{cryptoeprint:2016:168}.)
Then \engine fetches the requested datagram and returns it to \relay along with \dgform, \dgid, and a {\tt GASLIMIT} $\gasdeliver := \constgasmax$, all digitally signed with \skTC.
...
An honest requester first follows the protocol in Fig.~\ref{fig:att_check} to verify the SGX attestation.
...
We defer discussion of implementation details and other practical considerations to Appendix \ref{sec:impl} of the online version~\cite{cryptoeprint:2016:168}.
