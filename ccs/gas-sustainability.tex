...
% (Ethan:) I'm working on rewriting and formalizing things. This is scratch space for now.
%          I'm going to replace what's there after I get this through,
%          but since I'm editing things on 3 different computers right now, I want to get something partial committed.
%
...
%Without careful design, such services run the risk of malicious users draining financial resources by provoking blockchain transactions for which the service will not be reimbursed.
%In turn, this would lead to the service being unable to serve legitimate requests.
%It is therefore important the Ethereum-based services ensure that they will always be reimbursed for blockchain computation they initiate.
%
...
%
%To ensure that a service is not vulnerable to such attacks, we define a notion of \emph{gas sustainability}.
%Any service which is gas sustainable will always recoup any expended transaction fees.
%First we must define what a service consists of in this context.
%For simplicity, we will assume smart contracts operate on the Ethereum blockchain, but we emphasize that this is not inherent.
%Notationally, we let $(f(x), W, g)$ denote the wallet $W$ calling the contract function $f$ with input $x$ and supplying $g$ as the {\sf GASLIMIT}.
%
%\begin{definition}[Blockchain Service]
%A \emph{blockchain service} is a set of smart contract functions $f_1, \dotsc, f_n$,
%a set of wallets $\wallet_1, \dotsc, \wallet_m$,
%and a predicate $p$ on sequences of requests with the following properties.
%\begin{enumerate}
%  \item If ${\cal S} = \{(f_{i_1}(x_1), W_1, g_1), \dotsc, (f_{i_N}(x_N), W_N, g_N)\}$ with $W_j \neq \wallet_k$ for any $k$,
%    then there exists a (possibly empty) ${\cal S}' = \{(f_{j_1}(y_1), \wallet_{k_1}, h_1), \dotsc, (f_{j_M}(y_M), \wallet_{k_M}, h_M)\}$
%    such that $p({\cal S} \| {\cal S}') = {\sf true}$.
%  \item If ${\cal S}_1$ and ${\cal S}_2$ are sequences such that $p({\cal S}_1)$ and $p({\cal S}_2)$, then $p({\cal S}_1 \| {\cal S}_2)$.
%\end{enumerate}
%\end{definition}
