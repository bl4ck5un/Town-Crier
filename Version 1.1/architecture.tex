...
%  \node[rectangle,draw=black,trusted,right=2.75em of tc,text width=4.5em,minimum height=9.75em,align=center,label=above:{\bf Data Source}] (data) {\tt lots-o-\\data.com};
  \node[cloud,draw=black!65,trusted,cloud puffs=11,cloud puff arc=100,aspect=0.6,inner xsep=-0.35em,right=2.5em of tc,text width=4.15em,align=center] (data) {\tt lots-o-\\data.com};
  \node[minimum height=9.5em,minimum width=4em,label=above:{\bf Data Source}] () at (data) {};

  \draw[<->,comm-link] ([xshift=-0.75em,yshift=-0.75em]enc.north) |- ([xshift=-0.75em,yshift=-0.75em]ctc.east);
  \draw[<->,comm-link] ([yshift=0.75em]ctc.south) -- ([yshift=-0.75em]cu.north);
  \draw[<->,comm-link] ([xshift=0.75em,yshift=-0.75em]enc.north) |- ([xshift=1.1em,yshift=1.75em]data.west);
\end{tikzpicture}
...
\paragraph{Security challenges.} In addition to the constraints of this security model, we highlight two key technical challenges arising in the design of \tc. First, while data sources are presumed trustworthy by clients, they \emph{do not digitally sign their data}. Data sent over HTTPS can be authenticated by a client, but not by a third party. Second, while data on the blockchain can be authenticated using a full (Ethereum) client, we regard running such a client in the SGX enclave for \tc as impractical, as it would create a bloated and fragile TCB. Thus we face the challenge of ensuring that without direct blockchain authentication \tc can nonetheless authenticate messages originating on the blockchain.
