...
We now present our \tc architecture. First, we present a strawman architecture that illustrates some of the technical challenges that arise in the design of \tc. We then present the general structure of our \tc architecture, describing the interface available to a relying contract \reqcont, the various components of back end, and the security model underpinning our design.
...
request parameters, e.g.,  ${\sf params} := (\weburl, \dgform)$,
where $\weburl$ denotes an (HTTPS-enabled) data source URL and \dgform specifies the desired datagram.
Monitoring happens in the \medname, which controls the network stack. The
\medname forwards ${\sf params}$ 
to the \encname.
\item
{\bf Securely fetch feed.}
The \encname fetches the webpage at $\weburl$ by establishing
an HTTPS secure channel to the target page and host and verifying that the host's certificate matches the \weburl.
It fetches page contents, parses them, and extracts a datagram \dgm as specified in \dgform.

Finally, the \encname produces an attestation $\att$ on its executable, ${\sf params} := (\weburl, \dgform)$, a timestamp $T$ on the fetched data, and $\dgm$. 
It sends $\att$ to $\reqcont$.
\item
{\bf Verify}.
\reqcont verifies the attestation $\att$. If successful, \reqcont consumes \dgm (potentially matching {\sf params} against an internal queue of datagram requests).
\end{itemize}

This strawman scheme has a few nice features: conceptual simplicity, on-blockchain consumption of attestations, and no need to maintain a \tc contract on the blockchain. It has a few serious drawbacks, however.
