...
\begin{tabular}{m{0.13\columnwidth}m{0.8\columnwidth}}
  ...
  \hline
  $\gasdeliver$ $\gasrequest$ $\gascancel$
  & gas attached when invoking the {\bf Deliver}, {\bf Request}, {\bf Cancel} entries points respectively \\
  \hline
  $\constgasmin$
...
and $\gasdeliver$ is chosen by the TC \encname when calling {\bf Deliver}.
$\gasrequest$ and $\gascancel$ are set by the requester upon creating or cancelling a request,
but we need not worry about them being malicious because if they are too small
Ethereum will abort the transaction and it will be as if the entry point was never called.
...
We will prove in Section \ethan{refer} that this new protocol retains the correctness and authenticity guarantees from the payment-free protocol
while ensuring that Town Crier cannot lose gas when the \medname is honest and an honest requester's loss is limited even against a malicious Town Crier.
