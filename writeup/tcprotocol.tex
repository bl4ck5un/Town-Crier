...
A datagram request by \reqcont takes the form of a message $m_1 = (\{\dgid\}, \dgcallback, \dgform)$ to \tcont on the blockchain. Here, $\dgid$ is a unique request identifier assigned to the request by $\tcont$; we denote this assignment by $\{\,\}$, dropping brackets when better for clarity. The parameter $\dgcallback$ specifies the entry point in \reqcont to which the datagram is to be returned. (In principle, $\dgcallback$ could specify an entry point in a different contract, but \tc does not yet adopt this generalization. $\dgform$ specifies the requested datagram, e.g., ${\sf params} := (\weburl, {\sf spec}, T)$, where $\weburl$ is the target data source and {\sf spec} specifies content of a the datagram to be retrieved (e.g., a stock ticker at a particular time), while $T$ specifies the delivery time for the datagram. 
...
\subsection{Formal Guarantees: Authenticity of Data Feed}
