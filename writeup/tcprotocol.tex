...
\paragraph{The \encname \engine.} When initialized through an ``initialize'' call, \engine sets its absolute clock (by setting a reference point) and generates a keypair $(\pkTC,\skTC)$ to which it binds by placing $\pkTC$ in attestation requests. Given input ``resume'' $({\sf id}, {\sf params})$, \engine contacts the requested data source via HTTPS and checks that the corresponding certificate {\sf cert} is valid, i.e., not expired. We defer discussion of certificate revocation to Section~\ref{}. \engine then fetches the requested datagram and returns it to \relay along with the inputs, all digitally signed. 
...
{\bf Initialize}:  On receive ``initialize'', $T_0$: \\ %{\it //~called only once upfront}\\
\quad Set clock reference point; record $T_0$
\quad $(\pkTC, \skTC) := \Sigma.{\sf KeyGen}(1^\lambda)$\\
\quad Record $(\pkTC, \skTC)$\\
\\[1pt]

{\bf Attest}:  On receive ``attest'': \\ %{\it //~called only once upfront}\\
\quad $T := \clock()$\\
\quad Call quoting enclave with supp.~data $(\pkTC, T)$
\\[5pt]

{\bf Resume:} On receive ``resume'', $({\sf id}, {\sf params})$\\
...
\paragraph{The \medname \relay.} As noted in Section~\ref{sec:architecture}, \relay performs three types of data passing. To be a little more precise here, it scrapes the blockchain, monitoring \tcont for new datagram requests $({\sf id}, {\sf params})$. It boots the \engine with an ``initialize'' call and handles incoming requests by invoking \engine with ``attest'' or ``resume''  
...
{\bf  Loop forever}: \\
When \tcont receives new request $({\sf id}, {\sf params})$:\\
%\quad Parse ${\sf params} := (\weburl, \dgspec, T)$\\
\quad Fork: \\
\ \quad Send $(\text{``resume''}, ({\sf id}, {\sf params}))$ to $\fsgx[\enclaveprog, \relay]$ \\
\ \quad On recv $(({\sf id}, {\sf params}, {\sf data}), \sigma)$ from $\fsgx[\enclaveprog, \relay]$:\\ 
\ \quad \quad  {\sf AuthSend} $(({\sf id}, {\sf params}, {\sf data}), \sigma)$ to \tcont
\end{tabular}
\end{boxedminipage}
