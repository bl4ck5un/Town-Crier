...
In summary, then, we may assume in our protocol specifications that {\em all relying clients have verified an attestation for \encname and thus that datagram responses from \tcadd are trusted to originate from \engine.} 
...
\begin{tabularx}{\linewidth}{|@{\hspace{3pt}}r@{\hspace{1ex}}X@{\hspace{3pt}}|}
  ...
  {\bf Request:} & On recv $(\dgid, \dgform, \dgcallback)$ ({\bf msg.}~$m_1$) from $\reqcont$: \\
...
%                 & \hspace*{1em} Return $\${\sf fee}$ to $\pkU$ \\
  ...
  {\bf Deliver:} & On recv $(\dgid, \dgform, \dgm)$ from $\pkTC$: \\
		 ...
		 & Assert $\dgform = \dgform'$\\
                 & Call ${\dgcallback}({\dgm})$ ({\bf msg.}~$m_4$)\\
...
{\bf  Loop forever}: \\
When \tcont receives new request $({\sf id}, {\sf params})$ ({\bf msg.}~$m_2$):\\
%\quad Parse ${\sf params} := (\weburl, \dgspec, T)$\\
\quad Fork: \\
...
\ \quad \quad  {\sf Send} $(({\sf id}, {\sf params}, {\sf data}), \sigma)$ ({\bf msg.}~$m_3$) to \tcont from \tcadd 
\end{tabular}
\end{boxedminipage}
