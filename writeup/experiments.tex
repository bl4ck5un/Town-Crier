...
\addplot [dashed,brown,domain=1:20, samples=20, thick, forget plot]{4.9*x};
\addplot [dashed,blue,domain=1:20, samples=20, thick, forget plot]{2.00*x};
\addplot [dashed,red,domain=1:20, samples=20, thick]{1.60*x};
\addplot [mark=*,red,   thick, error bars/.cd,y dir=both,y explicit] table[x=n,y=steam-mean,y error=steam-std]{data.csv};
\addplot [mark=*,blue,  thick, error bars/.cd,y dir=both,y explicit] table[x=n,y=flight-mean,y error=flight-std]{data.csv};
\addplot [mark=*,brown, thick, error bars/.cd,y dir=both,y explicit] table[x=n,y=put-mean,y error=put-std]{data.csv};
...
for an enclave, and involves attestation generation.
Our experiments show that setting up the \tc~\encname takes
49.5(7.2) ms,
...
Recall also that since SGX reports only relative time 
with respect to some reference point, \tc's absolute clock is calibrated through a wall-clock timestamp furnished externally.
A user can verify the correctness of the \encname absolute block by requesting a digitally signed timestamp.
This measurement is, of course, accurate only to within the end-to-end latency for this verification procedure. Our experiments show that the time between \medname transmission of a clock calibration request to the enclave and receipt of a response is
11.4(1.9) ms, 
consumed mostly by signing the timestamp: 10.5(1.9) ms. 
