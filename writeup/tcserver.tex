...
\begin{tikzpicture}
  ...
  \draw[->,red,ultra thick,transform canvas={xshift=-2mm}] (https) to (net);
  \draw[->,red,ultra thick,dashed,transform canvas={xshift=+2mm}] (net) to (https);

  \draw[->,blue,ultra thick,transform canvas={xshift=-2mm}] (client) -- (att);
  \draw[<-,blue,ultra thick,dashed,transform canvas={xshift=+2mm}] (client) -- (att);

  \draw[->,blue,ultra thick,transform canvas={xshift=-2mm}] (monitor) -- (req h);
  \draw[<-,blue,ultra thick,dashed,transform canvas={xshift=+2mm}] (monitor) -- (req h);
  ...
  \path[->,color=red,ultra thick,transform canvas={yshift=0.1em},above] (ocall-leg.west) edge node {\small ocall} (ocall-leg.east);
  \path[<-,color=red,ultra thick,dashed,transform canvas={yshift=-0.4em}] (ocall-leg.west) edge (ocall-leg.east);

  \path[->,color=blue,ultra thick,transform canvas={yshift=0.1em},above] (ecall-leg.west) edge node {\small ecall} (ecall-leg.east);
  \path[<-,color=blue,ultra thick,dashed,transform canvas={yshift=-0.4em}] (ecall-leg.west) edge (ecall-leg.east);
\end{tikzpicture}
