...
$\sbrown{\$ {\tt G_{\text{deliver}}}}$ )$ from $\tcadd$: \\
	& If ${\sf bCanceled}[{\sf id}]$ and not ${\sf bDelivered}[{\sf id}]$:\\
		& \quad Send $\smaroon{\$F_{\text{cancel}}}$ to $\tcadd$  \\
		& \quad Set ${\sf bCanceled}[{\sf id}]$ and return\\

                 & Let $({\sf id}, {\sf params'}, {\sf callback}, \smaroon{\$ F'_{\text{deliver}}}, \_)$ be the first recorded tuple for ${\sf id}$ \sgray{\it //~abort if not found}\\
% & Assert ${\color{blue} \Time_{\text{timeout}}} > {\sf cur\_time}$ \\
                 & Assert ${\sf params} = {\sf params}'$\\
                &   Assert $\smaroon{\$ F'_{\text{deliver}}} \leq \sbrown{\$ G_{\text{deliver}}}$\\
...
%   & Assert ${\sf cur\_time} \geq {\color{blue} \Time_{\text{timeout}}}$\\
  ...
  & Send $\smaroon{\$ \overline{F}_{\text{deliver}}}$ to \reqcont\\
  \hline
\end{tabularx}
\caption{
