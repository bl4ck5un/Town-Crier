...
\begin{tabular}{m{0.08\columnwidth}m{0.85\columnwidth}}
  ...
  \hline
  $\constgasmax$
  & Maximum gas \tc can spend to invoke {\bf Deliver} \\
  \hline
...
During the creation and fulfillment of any request, there are two untrusted parties: The requester contract / user $\reqcont$ and the \tc~\medname. In the \tc implementation, we thus consider three cases, here giving the desired properties of each:
...
%  \setlength{\parskip}{0pt}
  ...
  \item {\it \tcs blockchain contract.}
    Figure \ref{tbl:gas-tc-contract} specifies the \tcs blockchain contract, with fees.
...
Our basic security model for \tc assumes the ideal isolation model for SGX described above, as well as client trust in data sources. Given various concerns about SGX security~\cite{sgxexplained,7163052}, and the possible fallibility of data sources, however, \tc can support two important forms of hedging. To protect against the compromise of a single SGX instance, contracts may request datagrams from multiple SGX instances and implement majority voting among the responses at the cost of increased gas expenditure for the additional requests and storing the returned datagrams.  Similarly, \tc can hedge against the compromise of a data source by scraping multiple sources for the same data and selecting the majority response. We demonstrate both of these mechanisms in our example financial derivative application in Section~\ref{subsec:hedging}. (A potential optimization is mentioned in Appendix~\ref{sec:future}.)
