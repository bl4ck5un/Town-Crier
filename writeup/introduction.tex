...
As Szabo's example shows, however, the most compelling applications of smart contracts require not just blockchain code, but access to data about real-world state and events. Similarly, financial contracts and derivatives, key applications for Ethereum~\cite{}, rely on data about financial markets, such as equity prices. %Applications such as insurance policies are only realizable with data about weather, flights, delivery of goods, and so forth. 
...
\item
{\it Gas neutrality (bounded \tc loss).} \ethan{zero \tc loss?} Assuming that $\tc$ executes honestly, it does not lose money (cannot suffer resource depletion)
...
\ethan{I'd like to discuss the presentation of this gaurantee. I reframed it a bit in Section 6 because ``loss'' is poorly specified.}
\end{itemize}
\vspace{-1mm}

To obtain the above formal guarantees,
we rely on the formal modeling 
of blockchains proposed by Kosba et al.~\cite{hawk} and the formal abstraction for SGX proposed by Shi et al. \elaine{cite}.
...
\elaine{mention range of response time}
\elaine{say that even a single SGX can support the throughput of an
entire blockchain}
