...
\begin{tikzpicture}
  ...
  \node[local-entity,trusted] (ctc) {TC Contract\\$\tcont$};
  \node[local-entity,trusted,right=7em of ctc] (enc) {Enclave\\$\enclaveprog$};
  ...
  \begin{pgfonlayer}{background}
    \node[bg-box,
          blockchain-color,
          fit={($(ctc.north west)+(-0.8em,0.6em)$)($(cu.south east)+(0.8em,-0.6em)$)},
          label=above:{\bf Blockchain}] () {};
    \node[bg-box,
          tc-server-color,
          fit={($(enc.north east)+(0.8em,0.6em)$)($(enc.south west)+(-0.8em,-0.6em)$)},
