...
%                 & \hspace*{1em} Return $\smaroon{\$fee}$ to $\pkU$ \\
		& Assert ${\color{blue} T_{\text{timeout}}} > {\sf cur\_time}$\\
                 & Record $({\sf id}, {\sf callback}, {\sf params}, \smaroon{\$ F_{\text{deliver}}}, {\color{blue} T_{\text{timeout}}}, \pkU)$\\[-10pt]
...
$\sbrown{\Delta {\tt F_{\text{deliver}}}}$ )$ from $\pksgx$: \\
                 & Let $({\sf id}, {\sf callback}, {\sf params'}, \smaroon{\$ F'_{\text{deliver}}}, {\color{blue} T_{\text{timeout}}}, \_)$ be the most recently recorded tuple for ${\sf id}$\\
 & Assert ${\color{blue} T_{\text{timeout}}} > {\sf cur\_time}$ \\
                 & Assert ${\sf params} = {\sf params}'$\\
...
from some user $\pkU$\\
  & Let $({\sf id}, \_, \_, \smaroon{\$ F_{\text{deliver}}}, {\color{blue} T_{\text{timeout}}}, \pkU')$ be the most recently recorded tuple for ${\sf id}$ \\
   & Assert $\pkU = \pkU'$\\
   & Assert ${\sf cur\_time} \geq {\color{blue} T_{\text{timeout}}}$\\
