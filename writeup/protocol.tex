...
%
%  \path[-stealth,communication] (cu) edge [transform canvas={xshift=-1.8em}] node [text width=4em,align=center,blockchain-color] {\footnotesize $m_1 =$\\[-0.2em]$(\dgform,$\\[-0.2em]$\dgcallback)$} (ctc);
...
%  \path[-stealth,communication] (enc) edge [below,transform canvas={yshift=-0.6em}] node [text width=4em,align=center] {\footnotesize $m_3 =$\\[-0.2em]$(\dgid,\dgform,$\\[-0.3em]$\dgm)$} (ctc);
%  \path[-stealth,communication] (ctc) edge [transform canvas={xshift=1.8em}] node [text width=3em,align=center,blockchain-color] {\footnotesize $m_4 =$\\[-0.4em]$(\dgid,\dgm)$} (cu);
%
%  \begin{pgfonlayer}{background}
%    \node[bg-box,
%          blockchain-color,
%          fit={($(ctc.north west)+(-0.75em,0.3em)$)($(cu.south east)+(0.75em,-0.3em)$)},
