...
\begin{boxedminipage}{\columnwidth}
\begin{center}
{\bf Town Crier's untrusted relay}
\end{center}
\begin{tabular}{l}
{\bf Initialize}:\\
Send ``initialize'' to $\fsgx[\enclaveprog]$\\
On receive $(\pksgx, \sigatt)$ from $\fsgx[\enclaveprog]$:\\
\quad Publish $(\pksgx, \sigatt)$\\[5pt]

{\bf  Loop forever}: \\
When $\Ctc$ receives new request $({\sf id}, \_, {\sf params})$:\\
\quad Parse ${\sf params} := (\weburl, \pkurl, T)$\\
\quad At time $T$: send ${\sf params}$ to the $\fsgx[\enclaveprog]$ \\
\quad On receive $({\sf data}, \sigma)$ from $\fsgx[\enclaveprog]$:\\ 
\quad \quad  {\sf AuthSend} $({\sf id}, {\sf data})$ to $\Ctc$
\end{tabular}
\end{boxedminipage}
